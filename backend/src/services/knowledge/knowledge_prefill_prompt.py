...
- knowledge_type: fact|certificate|template|solution|case|table|image（内容形态）
...
- expire_date: 由 qualification_info 有效期自动推导，输出时可省略
...
【knowledge_type 选型】
- certificate: 证书/资质/授权书/许可证/营业执照等证照类内容（与 block_type 独立，资质章节优先选 certificate）
- fact: 非证照类可核验事实（人员信息、数据指标、门店地址等）
- template: 固定格式模版正文
- solution: 方案、实施计划类叙述
- case: 案例、业绩描述
- table: 以表格为主体的内容
...
【有效期】
资质/证书/授权类：从正文提取 qualification_info；无明确日期留 null，不要猜测；不确定日期时 date_confidence 用 medium 或 low。
...
regions, qualification_info, date_confidence, is_template
"""
