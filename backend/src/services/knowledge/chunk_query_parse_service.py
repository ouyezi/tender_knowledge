...
_SYSTEM_PROMPT = (
    "你是知识块检索助手。将用户的自然语言搜索意图拆分为结构化 JSON。\n"
...
def parse_search_query(*, query: str) -> dict[str, Any]:
    ...
    if len(text) > settings.chunk_search_parse_query_max:
...
def _chat_with_timeout(*, user_prompt: str) -> str:
    model = settings.chunk_search_parse_model
    timeout_sec = settings.chunk_search_parse_timeout_sec
    ...
    try:
        ...
        logger.info("chunk search parse llm elapsed_ms=%.1f model=%s", elapsed_ms, model)
