...
Knowledge Base (kb)
  ...
  ├── Classification Reference (polymorphic, Epic 1+ writers)
...
**UNIQUE** `(kb_id, alias_normalized)` — 含标准名映射时可同步维护 canonical alias 行或单独校验 category_name 唯一性。
...
**UNIQUE** `(kb_id, synonym_normalized)` — 同义名全局不与其他 taxonomy 的标准名/同义名冲突。
...
| entity_id | UUID | |
| action | enum | create, update, alias_add, alias_remove, bind, unbind, deactivate, archive, merge |
| payload_summary | jsonb | 变更摘要 |
| created_at | timestamp | |
